...
function _M:init(text, key_source, force_all)
	...
	self.votex = core.renderer.white
...
function _M:setTexture(file)
	self.votex = core.display.loadImage(file):glTexture()
end
...
function _M:initGesturing()
	...
	self.vo = core.renderer.vertexes():texture(self.votex)
	self.vorenderer = core.renderer.renderer("dynamic"):add(self.vo)
	self.gesturing = true
	self.gesture = ""
	self.lastupdate = os.time()
	self.distance = 0
...
function _M:reset()
	if self.vo then
		self.fading_vorenderer = self.vorenderer
		self.fading = 1
...
function _M:update()
	...
	return (self.vorenderer or self.fading_vorenderer) and true or false
end
...
function _M:display(display_x, display_y, nb_keyframes)
	...
	if self.vo then
		self.vorenderer:toScreen(display_x, display_y)
	end

	if self.fading_vorenderer then
		self.fading_vorenderer:color(1, 1, 1, self.fading):toScreen(display_x - (1 - self.fading) * 20, display_y - (1 - self.fading) * 20)
		self.fading = self.fading - nb_keyframes / 20
		if self.fading <= 0 then
			self.fading_vorenderer = nil
		end
	end
end
