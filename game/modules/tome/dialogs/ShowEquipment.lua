...
function _M:init(title, actor, filter, action)
	...
	self.c_main_set = Tab.new{title="Main Set", default=not actor.off_weapon_slots, on_change=function(s) if s then self:switchSets("main") end end}
	self.c_off_set = Tab.new{title="Off Set", default=actor.off_weapon_slots, on_change=function(s) if s then self:switchSets("off") end end}
