...
_M._no_save_fields = { _shader = true, __z_layers = true, _do_cache = true, _temp_data = true, ai_state_volatile = true }
...
function _M:makeMapObject(tiles, idx)
	...
	if ok then
		...
		if self.spriter then
			...
			if self.spriter[3] then
				for _, cm in ipairs(self.spriter[3]) do sp:characterMap(cm, true) end
			end
...
function _M:setMoveAnim(oldx, oldy, speed, blur, twitch_dir, twitch)
	if not self._mo then return end
	self._mo:setMoveAnim(oldx, oldy, speed, blur, twitch_dir, twitch)
	...
	for i = 1, #add_displays do
		if add_displays[i]._mo then
			add_displays[i]._mo:setMoveAnim(oldx, oldy, speed, blur, twitch_dir, twitch)
		end
	end
end
