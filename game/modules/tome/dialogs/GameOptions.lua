...
function _M:generateListUi()
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Graphic Mode#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Smooth creatures movement#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Twitch creatures movement and attack#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Smooth fog of war#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Interface Style#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#HUD Style#WHITE##{normal}#", status=function(item)
	...
	if self:isTome() and game.uiset:checkGameOption("log_lines") then	
		local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"The number of lines to display in the combat log (for the Classic HUD)."}
		list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Log lines#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Draw faint lines to separate each grid, making visual positioning easier to see.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Display map grid lines#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Font Style#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Font Size#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Log fade time#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Duration of flying text#WHITE##{normal}#", status=function(item)
	...
	if self:isTome() then
		if game.uiset:checkGameOption("icons_temp_effects") then
			local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Uses the icons for status effects instead of text.#WHITE#"}
			list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Icons status effects#WHITE##{normal}#", status=function(item)
		...
		if game.uiset:checkGameOption("icons_hotkeys") then
			local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Uses the icons hotkeys toolbar or the textual one.#WHITE#"}
			list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Icons hotkey toolbar#WHITE##{normal}#", status=function(item)
		...
		if game.uiset:checkGameOption("hotkeys_rows") then
			local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Number of rows to show in the icons hotkeys toolbar.#WHITE#"}
			list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Icons hotkey toolbar rows#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"When you activate a hotkey, either by keyboard or click a visual feedback will appear over it in the hotkeys bar.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Visual hotkeys feedback#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"When the player or an NPC uses a talent shows a quick popup with the talent's icon and name over its head.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Talents activations map display#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Size of the icons in the hotkeys toolbar.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Icons hotkey toolbar icon size#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Always show lore popup#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"If disabled items with activations will not be auto-added to your hotkeys, you will need to manualty drag them from the inventory screen.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Always add objects to hotkeys#WHITE##{normal}#", status=function(item)
...
#{italic}#You can also change this directly ingame by pressing shift+T.#{normal}##WHITE#]]}
		list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Tactical overlay#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Toggles between a normal or flagpost tactical bars.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Flagpost tactical bars#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Toggles between a bottom or side display for tactial healthbars.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Healthbars position#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"If disabled you will not get a fullscreen notification of stun/daze effects. Beware.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Fullscreen stun/daze notification#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"If disabled you will not get a fullscreen notification of confusion effects. Beware.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Fullscreen confusion notification#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Toggles advanced weapon statistics display.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Advanced Weapon Statistics#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Always display the combat properties of gloves even if you don't know unarmed attack talents.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Always show glove combat properties#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Always display combat properties of shields even if you don't know shield attack talents.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Always show shield combat properties#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"When you do a mouse gesture (right click + drag) a color coded trail is displayed.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Display mouse gesture trails#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"If enabled new quests and quests updates will display a big popup, if not a simple line of text will fly on the screen.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Big Quest Popups#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Enable the WASD movement keys. Can be used to move diagonaly by pressing two directions at once.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Enable WASD movement keys#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Sharpen Visuals, set to 0 to disable.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Sharpen Visuals#WHITE##{normal}#", status=function(item)
...
function _M:generateListGameplay()
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Defines the distance from the screen edge at which scrolling will start. If set high enough the game will always center on the player.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Scroll distance#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"If you loose more than this percentage of life in a turn, a warning will display and all key/mouse input will be ignored for 2 seconds to prevent mistakes.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Life Lost Warning#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Weather effects#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Enables or disables day/night light variations effects..#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Day/night light cycle#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Enables easy movement using the mouse by left-clicking on the map.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Use mouse to move#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Quick melee targeting#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Quick melee targeting auto attack#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Enables mouse targeting. If disabled mouse movements will not change the target when casting a spell or using a talent.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Mouse targeting#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Auto-accept target#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"New games begin with some talent points auto-assigned.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Auto-assign talent points at birth#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Always rest to full before auto-exploring.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Rest before auto-explore#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"When swaping an item with a tinker attached, swap the tinker to the newly worn item automatically.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Swap tinkers#WHITE##{normal}#", status=function(item)
...
function _M:generateListOnline()
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Configure the chat filters to select what kind of messages to see.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Chat message filters#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Configure the chat ignore filter.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Chat ignore list#WHITE##{normal}#", status=function(item)
		return "select to configure"
	end, fct=function(item)	game:registerDialog(require("engine.dialogs.ChatIgnores").new()) end,}

	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Configure the chat channels to listen to.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Chat channels#WHITE##{normal}#", status=function(item)
		return "select to configure"
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Open links in external browser#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Discord's Rich Presence#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Upload characters sheets to the online vault#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Allow online events#WHITE##{normal}#", status=function(item)
...
A full exit and restart of the game is neccessary to apply this setting.#{normal}#]]}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Disable all connectivity#WHITE##{normal}#", status=function(item)
...
function _M:generateListMisc()
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Save in the background#WHITE##{normal}#", status=function(item)
	...
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Zone save per level#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Disallow boot images that could be found 'offensive'.#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Censor boot#WHITE##{normal}#", status=function(item)
	...
	local zone = Textzone.new{width=self.c_desc.w, height=self.c_desc.h, text=_t"Replace headwear images by cloak hoods if a cloak is worn#WHITE#"}
	list[#list+1] = { zone=zone, name=_t"#GOLD##{bold}#Show cloak hoods#WHITE##{normal}#", status=function(item)
