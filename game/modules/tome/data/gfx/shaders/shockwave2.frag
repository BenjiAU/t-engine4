...
uniform float innerShockwaveWidth;
...
vec4 GetFireRingColor(float currTime, vec2 pos, float freqMult, float stretchMult, float ampMult, float power, float radius1, float radius2, float scrollSpeed, float twist, float paletteCoord)
{
	float pi = 3.141592;
	float ang = atan(pos.y, pos.x) + pi;
	
	vec2 planarPos = vec2(ang / (2.0 * pi), 1.0 - (length(pos) - radius1) / (radius2 - radius1));
	planarPos.x += planarPos.y * twist;
	planarPos.x = mod(planarPos.x, 1.0);
...
void main(void)
{
	...
	float innerWidth = innerShockwaveWidth / 2.0;
	float outerWidth = outerShockwaveWidth / 2.0;

	float phase = max(0.0, min(1.0, ((tick - tick_start) / time_factor * shockwaveSpeed)));
	float ringRadius = (0.5 - outerWidth) * (1.0 - pow(1.0 - phase, 2.0));
	
	vec2 dir = radius / radiusLen;
		
	
	vec4 c;
	if(radiusLen > ringRadius - innerWidth && radiusLen < ringRadius)
	{
		c = GetFireRingColor(tick / time_factor +  0.0 , radius, 3.0, 25.0, flameIntensity, 1.0, ringRadius, ringRadius - innerWidth, 0.0, 0.1, 0.5);
	}else
	if(radiusLen > ringRadius && radiusLen < ringRadius + outerWidth)
	{
		c = GetFireRingColor(tick / time_factor +  0.0 , radius, 5.0, 80.0, flameIntensity, 2.0, ringRadius, ringRadius + outerWidth, 0.0, 0.0, 0.5);
	}else
	{
