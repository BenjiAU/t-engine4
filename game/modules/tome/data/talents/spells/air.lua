...
newTalent{
	...
	action = function(self, t)
		...
		for i, actor in ipairs(targets) do
			...
			game.level.map:particleComposeEmitter(sx, sy, math.max(math.abs(actor.x-sx), math.abs(actor.y-sy)), "lightning_beam", {tx=(actor.x-sx)*Map.tile_w, ty=(actor.y-sy)*Map.tile_h}, rng.float(0.8, 1.2))
