...
void Navmesh::drawDebug(float x, float y) {
	...
	auto dl = getDisplayList(renderer, {(texture_do){(GLuint)gl_tex_white, GL_TEXTURE_2D}, 0, 0}, NULL, VERTEX_MAP_INFO, RenderKind::TRIANGLES);
	for (auto tri : mesh) {
	...
	dl = getDisplayList(renderer, {(texture_do){(GLuint)gl_tex_white, GL_TEXTURE_2D}, 0, 0}, NULL, VERTEX_MAP_INFO, RenderKind::LINES);
	for (auto tri : mesh) {
