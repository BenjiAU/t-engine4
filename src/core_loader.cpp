...
public:
	LoaderPNGCubemap(array<string, 6> &filenames, array<PHYSFS_file*, 6> &files, texture_type *tex) : tex(tex) {
		for (int i = 0; i < 6; i++) {
	...
	virtual bool finish() {
		if (s[0] && s[1] && s[2] && s[3] && s[4] && s[5]) {
			printf("[LOADER] done loading cubemap PNG %s / %s / %s / %s / %s / %s in texture %d!\n", filenames[0].c_str(), filenames[1].c_str(), filenames[2].c_str(), filenames[3].c_str(), filenames[4].c_str(), filenames[5].c_str(), tex->tex);
			tfglBindTexture(GL_TEXTURE_CUBE_MAP, tex->tex);
...
static int lua_loader_cubemap_png(lua_State *L) {
	...
	auxiliar_setclass(L, "gl{texture}", -1);
	glGenTextures(1, &t->tex);
	tfglBindTexture(GL_TEXTURE_CUBE_MAP, t->tex);
